...
function Canvas:get_pixel(row, col)
  return self.canvas[row][col][1], self.canvas[row][col][2], self.canvas[row][col][3], self.canvas[row][col][4],
         self.canvas[row][col][5], self.canvas[row][col][6], self.canvas[row][col][7], self.canvas[row][col][8],
         self.canvas[row][col][9]
end
...
function Canvas:row_col_min_max()
  ...
  if DEBUG then print("Canvas row[" .. rmin .. ".." .. rmax .. "] col[" .. cmin .. ".." .. cmax .. "]") end
  return rmin, rmax, cmin, cmax
end

function Canvas:new_canvas_cropped_with_stroke()
  local rmin, rmax, cmin, cmax = self:row_col_min_max()
  newcanvas = Canvas(rmax - rmin + 1 + 2, cmax - cmin + 1 + 2) -- adding +1 for indexing from 1, +2 or one extra pixel on each edge
...
function concat_canvases(sprite_canvases, separation)
  ...
  for i, sprite in ipairs(sprite_canvases) do
    ...
    if sprite.rows > max_rows then max_rows = sprite.rows end
  end

  local s = Canvas(max_rows, total_cols)
          ...
          if col <= self.screen_width then
            ...
            if transparency then
              if c == EMPTY then
                ...
                local fg, bg
                if COLORS_256 then local fg = self:fg256(b256) end
...
function ship:draw_sprite_rotated(canvas, angle, pos)
  ...
  screen_position:add(Vector(ceil(self.sprite_rows / 2), ceil(self.sprite_columns / 2)))
  ...
  for y = 1, rows do
    ...
    for x = 1, cols do
      ...
      if color ~= nil and color ~= tcolor then
        local pixel1 = Vector(x - floor(cols / 2) - 1, rows - y - floor(rows / 2))
...
function planet:new(x, y, phase, r, ptype)
  ...
  self.noise_factor_vert = random_int(planet_type.max_noise_stretch_factor + 1, planet_type.min_noise_stretch_factor)
