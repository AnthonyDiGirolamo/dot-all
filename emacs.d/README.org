...
- [[#corfu-auto-completion][corfu auto-completion]]
...
- [[#completion-frameworks][completion frameworks]]
  - [[#minibuffer---vanilla-emacs][minibuffer - vanilla emacs]]
  - [[#orderless][orderless]]
  - [[#consult][consult]]
  - [[#embark][embark]]
  - [[#vertico][vertico]]
...
#+begin_src emacs-lisp
  ...
  (use-package emacs
    ...
    :custom
    ;; START vertico recommended config
    ...
    ;; END vertico recommended config

    :config
    ;; Erase trailing whitespace before save
    (add-hook 'before-save-hook 'delete-trailing-whitespace)

    ;; chmod +x scripts that start with #!
...
#+begin_src emacs-lisp
  ...
  (defun amd/clear-and-load-theme (theme-name)
    "Disable all loaded themes and load the provided theme."
    (amd/disable-loaded-themes)
    (load-theme (intern theme-name) t))

  (defun amd/load-theme ()
    "Interactively load a theme."
    (interactive)
    (let ((this-command 'amd/load-theme))
      (ivy-read
       "Switch theme: "
       (custom-available-themes)
       :action #'amd/clear-and-load-theme)))

  (defun amd/set-variable-org-level-heights ()
    "Set variable font heights for org-levels "
    (interactive)
    (custom-set-faces
...
#+begin_src emacs-lisp
  (use-package doom-themes
    ...
    (defun amd/doom-palenight-theme ()
...
#+begin_src emacs-lisp
  ...
  (defun amd/x-paste ()
    "Paste from the clipboard with wl-paste."
    (interactive)
    (insert (shell-command-to-string "wl-paste")))

  (defun amd/x-yank (begin end)
    "Yank to the x clipboard with wl-copy."
    (interactive "r")
    (shell-command-on-region begin end "wl-copy -p"))
...
#+begin_src emacs-lisp
  (use-package evil
    ...
    (evil-undo-system 'undo-redo) ;; for vundo
    ...
    (:states '(insert)
     ...
     amd/leader-key-insert 'amd/transient-leader-menu)
    ...
    (:states '(motion visual emacs)
     ...
     amd/leader-key 'amd/transient-leader-menu)
    (:states '(motion emacs)
     "C-M-c" 'amd/split-compilation
...
#+begin_src emacs-lisp
  (use-package proced
    ...
    (:states '(emacs)
     ...
     "/" 'swiper
     "n" 'next-line
     "g" (lambda() (interactive) (revert-buffer) (recenter))
     "e" 'previous-line)
   :custom
   (proced-sort 'pid)
   (proced-tree-flag t))
...
#+begin_src emacs-lisp
  ...
  (defun multiply-number-at-point (factor)
    "Multiply the number at point by FACTOR using `calc-eval` and replace it.
  If called with a prefix argument (e.g., C-u 3 M-x multiply-number-at-point),
  uses that as the factor; otherwise, prompts in the minibuffer."
    (interactive
     (list (if current-prefix-arg
               (prefix-numeric-value current-prefix-arg)
             (read-number "Multiply by: "))))
    (let ((bounds (bounds-of-thing-at-point 'number))
          (num (number-at-point)))
      (if (null bounds)
          (user-error "No number found at point")
        (let* ((num-str (number-to-string num))
               (factor-str (number-to-string factor))
               (expr (format "%s * %s" num-str factor-str))
               (result (calc-eval expr)))
          (delete-region (car bounds) (cdr bounds))
          (insert result)))))

  (defun divide-number-at-point-by-two ()
    (interactive)
    (multiply-number-at-point 0.5))

  (defun divide-number-at-point-by-four ()
    (interactive)
    (multiply-number-at-point 0.25))
#+end_src
...
#+begin_src emacs-lisp
  (use-package info-mode
    ...
    (:states '(motion) :keymaps '(Info-mode-map)
      ...
      ;; Not sure why these need redefining here and are not inherited
      ;; from evil default motion map.
      "C-l" (lambda() (interactive) (evil-ex-nohighlight) (redraw-display))
...
#+begin_src emacs-lisp
  ...
  (use-package company-quickhelp
    :disabled
    :if window-system  ;; doesn't work on the console and overwrites M-h keybinding
    :init
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    ;; use c-j to jump to live file from magit-status. Or set this:
...
#+begin_src emacs-lisp
  (use-package vundo
    ...
    (setq vundo-window-max-height 3)
    ...
    :bind ("C-x u" . vundo)
    :config
    (add-to-list 'evil-emacs-state-modes 'vundo-mode)
    (add-to-list 'evil-emacs-state-modes 'vundo-diff-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package shackle
    ...
    ;; WARNING: This default rule causes transients to always appear
    ;;          on the right side if there is only one window in the frame.
    ;;   (shackle-default-rule '(:select t))
    (shackle-default-rule nil)
    (shackle-default-alignment 'below)
    (shackle-default-size 0.5)
    (shackle-trace-functions
     '(display-buffer
       pop-to-buffer
    ...
    (setq
     ...
     '(;; Hidden popups
       ...
       ("*Warnings*" :select nil :ignore t :other t)
       ...
       ;; ('vundo-mode :align below :same t)
       ;; ("\\*vundo-diff.*\\*" :align below :same t)
...
For lsp-mode to work with rust or clangd it works best if these files
are available in the project root folder:

- ~rust-project.json~ is available or symlinked

- ~rustc~ and ~rust-analyzer~ are available on the path

- ~compile_commands.json~ is available or symlinked

- Create an empty ~.projectile~ file in the project root.

- Create a ~.dir-locals.el~ with the following (as an example). This
  sets the local clangd executable to one relative to the project
  root.
     ...
     (nil
      . ((eval
          . (add-to-list 'exec-path
                         (expand-file-name
                          (concat (projectile-project-root) "/prebuilt/third_party/clang/linux-x64/bin"))))
         (lsp-clients-clangd-args . ("--background-index=true"))
         (eval
          . (setq lsp-clients-clangd-executable (expand-file-name (concat (projectile-project-root) "/prebuilt/third_party/clang/linux-x64/bin/clangd"))))
         )
      )
     (magit-status-mode
      . ((eval
...
#+begin_src emacs-lisp
  ...
  (use-package mmm-jinja2
    :config
    (add-to-list 'auto-mode-alist '("\\.rs.jinja\\'" . rust-mode))
    (mmm-add-mode-ext-class 'rust-mode "\\.rs.jinja'" 'jinja2))
...
#+begin_src emacs-lisp
  (use-package transient-posframe
    ...
    (setq transient-posframe-border-width 0)
    (setq transient-posframe-poshandler #'posframe-poshandler-frame-bottom-left-corner)
    (setq transient-display-buffer-action '(transient-posframe--show-buffer))
    )
#+end_src
...
#+begin_src emacs-lisp
  ...
  (pretty-hydra-define hydra-set-toggles
    (:color blue :quit-key "q"
            ;; TODO: this formatter broke sometime recently
            ;; :formatter (lambda (docstring) (replace-regexp-in-string "\n\\([^\n]+\n\\)[-=─]+\n" "\\1" docstring))
            )
    ("Line Endings"
  ...
  (amd/defhydra-with-columns
   ...
   `((("d" counsel-find-file      "find-file")
     ...
     (("ps" projectile-switch-project        "project switch")
      ...
      ("rf" consult-recent-file              "recentf")
  ...
  ;; (define-key evil-normal-state-map amd/leader-key 'amd/transient-leader-menu)
  ;; (define-key evil-motion-state-map amd/leader-key 'amd/transient-leader-menu)
  ;; (define-key evil-visual-state-map amd/leader-key 'amd/transient-leader-menu)
...
#+begin_src emacs-lisp
  (use-package transient
    :demand
    :init
    ;; Show transient pop-up directly below the focused window.
    ...
    ;; The default is the bottom of the screen (frame):
    ;; (display-buffer-in-side-window (side . bottom) (dedicated . t) (inhibit-same-window . t))
    :config
    (defun amd/major-mode-leader-transient ()
      (interactive)
      (cond
       ((eq major-mode 'org-mode)
        (amd/transient-org-menu))

       ((or (eq major-mode 'lisp-interaction-mode)
            (eq major-mode 'emacs-lisp-mode))
        (amd/transient-elisp-menu))

       ((eq major-mode 'rust-mode)
        (amd/transient-rust-menu))

       ((or (eq major-mode 'nxml-mode)
            (eq major-mode 'xml-mode))
        (amd/transient-xml-menu))
    ...
    (transient-define-prefix amd/transient-rust-menu ()
      "rust-mode leader menu"
      :transient-non-suffix #'transient--do-exit
      [["Functions"
        ("t" "test" rust-test)
        ("k" "check" rust-check)
        ("l" "clippy" rust-run-clippy)
        ("r" "run" rust-run)
        ("f"  "format" rust-format-buffer)
        ("," "Quit" transient-quit-one)]])

    (transient-define-prefix amd/transient-elisp-menu ()
      "emacs-lisp-mode leader menu"
      :transient-non-suffix #'transient--do-exit
      [["Functions"
        ("e" "[org] toggle code edit" amd/toggle-org-edit-code-block)
        ("d" "eval defun"             eval-defun)
        ("D" "eDebug defun"           amd/edebug-eval-defun)
        ("," "Quit" transient-quit-one)]])

    (transient-define-prefix amd/transient-xml-menu ()
      "xml-mode leader menu"
      :transient-non-suffix #'transient--do-exit
      [["Functions"
        ("f" "xml format"
         (lambda () (interactive)
    ...
    (transient-define-prefix amd/transient-align-menu ()
      "Align Menu"
      [["Characters"
        (":" "align :" align-to-colon)
        ("=" "align =" align-to-equals)
        ("," "align ," align-to-comma)
        ("s" "align whitespace" align-to-space)]
       ;; Group 2: Buffer Actions
       ["Regex"
        ("r" "align repeat" align-repeat)
        ("n" "align no-repeat" align-no-repeat)
        ("i" "align interactive" align-interactively)]])

    (transient-define-prefix amd/transient-org-menu ()
      "org-mode leader menu"
      :transient-non-suffix #'transient--do-exit
      [["Capture"
        ("cc" "capture" org-capture)
        ("cl" "store link" org-store-link)
        ("ci" "clock in" org-clock-in)
        ("co" "clock out" org-clock-out)
        ("cd" "clock display" org-clock-display)
        ("o" "open todos" (lambda() (interactive) (find-file org-default-notes-file)))]

       ["Formula"
        ("fc" "formula coords" org-table-toggle-coordinate-overlays)
        ("fd" "formula debug" org-table-toggle-formula-debugger)
        ("fe" "formula edit"
         (lambda() (interactive)
           (let ((current-prefix-arg '(16))) ;; emulate C-u C-u
             (call-interactively 'org-table-eval-formula)
        ...
        ("tp" "tangle at point"
         (lambda() (interactive)
           ...
           (org-babel-tangle)
           (widen)))
        ("ta" "tangle all" org-babel-tangle)]
       ...
       ["Headings"
        ...
        ("ih" "insert header" org-babel-insert-header-arg)]

       ["Agenda/Edit"
        ("a" "agenda" org-agenda)
        ("y" "archive subtree" org-archive-subtree)
        ("R" "restart" org-mode-restart)
        ("l" "change tag (label)" counsel-org-tag)
        ("e" "code edit" amd/toggle-org-edit-code-block)]

       ["Trees"
        ("tt" "todo tree" org-show-todo-tree)
        ("A" "show all"
         (lambda() (interactive) (show-all) (org-remove-occur-highlights)))
        ("w" "widen" widen)
        ("n" "subtree" org-narrow-to-subtree)
        ("gl" "tag tree" org-match-sparse-tree)]

       ;; ["Shift"
       ;;  ("h" "←" org-shiftmetaleft  :transient t)
       ;;  ("l" "→" org-shiftmetaright :transient t)
       ;;  ("n" "↓" org-shiftmetadown  :transient t)
       ;;  ("e" "↑" org-shiftmetaup    :transient t)]

       ["Export"
        ;; ("d" "pandoc" pandoc-main-hydra/body)
        ("hy" "org→html→yank" amd/clipboard-org-buffer-to-html-xclip)
        ("hp" "html→org→paste" amd/clipboard-html-to-org-pandoc-linux)
        ("x" "export" org-export-dispatch)
        ("," "Quit" transient-quit-one)]])

    (defun amd/tab-bar-string-for-transient ()
      "Return current tabs as a formatted string."
      (cl-loop
       for tab being the elements of (funcall tab-bar-tabs-function)
       using (index i)
    ...
    (defun amd/display-buffer-in-posframe (buffer _alist)
      "Show BUFFER in posframe."
    ...
    (transient-define-prefix amd/transient-tab-bar-menu ()
      "tab-bar-mode leader menu"
      ;; pressing any other key closes the transient
      :transient-non-suffix #'transient--do-exit
      ;; show in a posframe to not leave old transient buffers
      :display-action '(amd/display-buffer-in-posframe)
      [
       :description (lambda () (amd/tab-bar-string-for-transient))
       [("h" "← prev"           tab-previous :transient t)
        ("H" "← move"           tab-bar-move-tab-backward :transient t)
        ("T" "split → new Tab"  tab-bar-move-window-to-tab :transient t)]

       [("l"  "→ next/new"       amd/tab-bar-next-or-new-tab :transient t)
        ("L"  "→ move"           tab-bar-move-tab :transient t)
        ("F"  "tab → new Frame"  tab-bar-detach-tab :transient t)]

       [("N"  "new"               tab-bar-new-tab :transient t)
        ("c"  "close"             tab-bar-close-tab :transient t)
        ("t"  "switch"            tab-bar-switch-to-tab)]

       [("o"  "reopen"            tab-bar-undo-close-tab :transient t)
        ("r"  "rename"            tab-rename :transient t)]

       [("w"  "window size"       amd/transient-window-size-menu)
        ("f"  "window frame"      amd/transient-frame-menu)
        ("RET" "Quit"             transient-quit-all)]
       ])

    (transient-define-prefix amd/transient-window-size-menu ()
      "window size leader menu"
      ;; pressing any other key closes the transient
      :transient-non-suffix #'transient--do-exit
      ;; show in a posframe to not leave old transient buffers
      :display-action '(amd/display-buffer-in-posframe)
      [[""
        ("h" "< shrink"  shrink-window-horizontally :transient t)
        ""]
       [("e" "↑ shrink"    shrink-window :transient t)
        ""
        ("n" "↓ enlarge"   enlarge-window :transient t)]
       [""
        ("l" "> enlarge" enlarge-window-horizontally :transient t)
        ""]
       [("w" "maximize"           evil-window-set-height :transient t)
        ("b" "balance"           balance-windows :transient t)
        ("RET" "Quit"             transient-quit-all)]
       ])

    (transient-define-prefix amd/transient-frame-menu ()
      "window size leader menu"
      ;; pressing any other key closes the transient
      :transient-non-suffix #'transient--do-exit
      ;; show in a posframe to not leave old transient buffers
      :display-action '(amd/display-buffer-in-posframe)
      ["Frame"
       [("m" "maximize"   toggle-frame-maximized :transient t)
        ("f" "fullscreen" toggle-frame-fullscreen :transient t)
        ("n" "new"        make-frame)
        ("c" "close"      delete-frame)
        ("RET" "Quit"     transient-quit-all)]
       ])

    (transient-define-prefix amd/transient-leader-menu ()
      ...
      :display-action '(display-buffer-at-bottom
                        ...
                        (inhibit-same-window . t))
      [["File"
        ("d" "find-file"                 counsel-find-file)
        ("g" "git"                       magit-status)
        ("s" "set-toggles"               hydra-set-toggles/body)
        ("u" "vundo"                     vundo)
        ("w" "window"                    ace-window)
        ("t" "tabs"                      amd/transient-tab-bar-menu)
        ("c" "calc"                      calc-dispatch)
        ("j" "jump-imenu"                consult-imenu)
        ("v" "open .emacs"               (lambda () (interactive) (find-file "~/dot-all/")))
        ("V" "open README.org"           (lambda () (interactive) (find-file amd/settings-org-file)))]

       ["Check"
        ("fc" "flycheck"                 flycheck-list-errors)
        ("fn" "flycheck next error"      flycheck-next-error)
        ("fp" "flycheck prev error"      flycheck-previous-error)
        ("bb" "buffer switch"            consult-buffer)
        ("bi" "buffer ibuffer"           ibuffer)
        ("bk" "buffer kill"              kill-current-buffer)
        ("br" "buffer revert"            revert-buffer)
        ("yb" "yank buffer"              amd/copy-buffer)
        ("yxy" "x-yank"                  amd/x-yank)
        ("yxp" "x-paste"                 amd/x-paste)]

       ["Edit"
        ("a" "align"                     amd/transient-align-menu)
        ("G"  "search git grep"          counsel-git-grep)
        ;; ("rgg"  "ripgrep"                counsel-rg)
        ("rg"  "ripgrep w/ dir"          amd/counsel-rg-with-initial-directory)
        ("pa" "search ag counsel"        counsel-ag)
        ("pf" "search ag filename regex" amd/counsel-ag-with-filename-regex)
        ;; ("pt" "search pt counsel"        counsel-pt)
        ;; ("pp" "search pt project"        projectile-pt)
        ;; ("po" "search pt other"          pt-regexp)
        ("/"  "search grep/swiper"       counsel-grep-or-swiper)
        ("," "major-mode hydra"          amd/major-mode-leader-transient)]

       ["Load"
        ("lt" "load theme"               load-theme)
        ("lc" "list colors"              list-colors-display)
        ("lf" "list faces"               list-faces-display)
        ("lp" "list packages"            package-list-packages)
        ("hk" "help keys"                counsel-descbinds)
        ("hK" "help whichkey"            which-key-show-top-level)
        ("hm" "help mode"
         (lambda () (interactive) (describe-variable 'major-mode)))
        ("bu" "browse url"               browse-url-generic)]

       ["Project"
        ("ps" "project switch"           projectile-switch-project)
        ("pe" "project eshell"           amd/eshell-or-project-eshell)
        ("pc" "project compile"          project-compile)
        ("pr" "project recompile"        project-recompile)
        ("pi" "project clear"            projectile-invalidate-cache)
        ("rf" "recentf"                  consult-recent-file)
        ("rt" "run test"                 run-current-test)
        ("rF" "run file"
         (lambda () (interactive) (run-current-test nil t)))
        ("rp" "run pico8"                run-pico8)
        ("rl" "run love2d"               run-love2d)]

       ["Desktop"
        ("zi" "zoom-in"
         (lambda () (interactive) (text-scale-increase 0.5)) :transient t )
        ("zo" "zoom-out"
         (lambda () (interactive) (text-scale-decrease 0.5)) :transient t )
        ("DS" "desktop-save"             desktop-save)
        ("DC" "desktop-clear"            desktop-clear)
        ("DL" "desktop-read"             desktop-read)
        ("zz" "server-edit done"         server-edit)
        ("WC" "word-count"               count-words)
        ("xx"  "M-x"                     counsel-M-x)]])

    ;; Truncate long lines in all transient buffers
    (add-hook 'transient-setup-buffer-hook (lambda () (setq truncate-lines t)))
    (transient-bind-q-to-quit))
#+end_src

* completion frameworks

** minibuffer - vanilla emacs

#+begin_src emacs-lisp
  (use-package minibuffer
    :ensure nil
    :config
    ;; Ignore case for files, buffers, and generally
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    :init
    (defun amd/consult-git-or-file-jump ()
      "Run counsel-git if in a git repo, counsel-file-jump otherwise."
      (interactive)
      (if (consult-ls-git--get-project-root)
...
#+begin_src emacs-lisp
  (use-package embark
    ;; :disabled
    :bind
    (("C-t" . embark-act)         ;; pick some comfortable binding
     ("C-c C-o" . embark-export)
     ("C-;" . embark-dwim)        ;; good alternative: M-.
     ("C-h C-k" . (lambda () (interactive) (embark-bindings t)))) ;; alternative for `describe-bindings'
...
#+begin_src emacs-lisp
  (use-package vertico
    ...
    :general
    (:keymaps '(vertico-map)
      ;; "C-n" 'vertico-next ;; this is the default
...
#+begin_src emacs-lisp
  (use-package swiper
    ...
    ;; Enable ivy for all completions
    ;; (ivy-mode 1)
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (defun amd/clipboard-html-to-org-pandoc-mac ()
      ...
      (kill-new
       (shell-command-to-string
        "osascript -e 'the clipboard as \"HTML\"' | ruby -ne 'puts([$_[10..-3]].pack(\"H*\"))' | pandoc -f html -t org"))
      (yank))
    ...
    (defun amd/clipboard-html-to-org-pandoc-linux ()
      ...
      (kill-new
       (shell-command-to-string
        ;; "xclip -selection clipboard -o -t text/html | pandoc -f html -t org"
        "wl-paste -t text/html | pandoc -f html -t org"))
      (yank))

    (defun amd/clipboard-html-to-markdown-pandoc-linux ()
      "Convert clipboard contents from HTML to Org and then paste (yank)."
      (interactive)
      (kill-new
       (shell-command-to-string
        ;; "xclip -selection clipboard -o -t text/html | pandoc -f html -t org"
        "wl-paste -t text/html | pandoc -f html -t markdown"))
      (yank))
