...
** ppimport
import pprint
_pp = pprint.PrettyPrinter(indent=1, width=120).pprint
_pf = pprint.PrettyPrinter(indent=1, width=120).pformat
...
#+begin_src fish
  ...
  # If args provided on the command line run that and exit.
...
- ~:h[digit]~ goes up one directory level
- ~:A~ makes the path absolute and resolves symlinks.
...
** echorun: echo a command and run it

#+begin_src sh
  ACTION_ARROW="\x1b[36m==>\x1b[0m"
  function echorun () {
      echo -e $ACTION_ARROW $*
      $*
  }
...
** dir-locals-lsp-path                                                 :src:

Create a ~.projectile~ file in the project root folder if needed for ~(projectile-project-root)~ to work correctly.
    ...
    . ((eval
       ...
       (eval
        . (setq lsp-clients-clangd-executable
                (expand-file-name (concat (projectile-project-root) "/some/local/project/path/bin/clangd"))))))
   (magit-status-mode
