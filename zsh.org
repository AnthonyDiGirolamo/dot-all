...
#+begin_src sh :tangle ~/.zshrc.local
  ...
  zstyle ':prompt:grml:left:setup' items rc shell-level change-root path vcs newline percent

  # Available prompt items are:
  #   at, battery, change-root, date, grml-chroot, history, host, jobs, newline,
  #   path, percent, rc, rc-always, sad-smiley, shell-level, time, user, vcs

  zstyle ':prompt:grml:right:setup' use-rprompt false
  zstyle ':prompt:grml:right:setup' items sad-smiley

  function amd_vcs_format() {
      local AF="%F{blue}%s%F{yellow}:%F{green}%b%F{yellow}|%F{red}%a%f "
      local F="%F{blue}%s%F{yellow}:%F{green}%b%%f "
      local BF="%b%F{red}:%F{yellow}%r"
      zstyle ':vcs_info:*'              actionformats "$AF" "zsh: %r"
      zstyle ':vcs_info:*'              formats       "$F"  "zsh: %r"
      zstyle ':vcs_info:(sv[nk]|bzr):*' branchformat  "$BF"
  }
